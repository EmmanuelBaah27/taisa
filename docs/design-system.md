...
| `BottomNavBar` | _(none)_ | Figma-state app navigation: 240×60 for Home/Chats and 220×60 for Me; one persistent selected icon/capsule and its centered shell width move on one coordinated 280ms React Native Animated curve while the entire glass shell scales uniformly to 1.12 in 90ms; handoff overlaps labels only; Me preserves the Navii avatar; native iOS glass with material-blur fallback |
