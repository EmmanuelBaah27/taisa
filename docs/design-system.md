...
| `BottomNavBar` | _(none)_ | Figma-state app navigation: 240×60 for Home/Chats and 220×60 for Me; all three tab identities remain mounted above a content-free moving glass capsule; each tab frame and its icon follow the same non-oscillating 260ms fluid curve while its full label fades and scales outward from the icon-facing edge over 160ms (never width-clipped); the 6% grey fill begins returning at 120ms and completes with arrival; navigation mounting waits until the 260ms travel completes so route rendering cannot interrupt it; no tab icon is hidden or exchanged; the shell peaks uniformly at 1.12 over 90ms and returns over the next 220ms during capsule travel; Me preserves the Navii avatar; native iOS glass with material-blur fallback |
