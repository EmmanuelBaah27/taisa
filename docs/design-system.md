...
| `PersistentNavigationCapsule` | `label`, `leadingVisual`, `frame`, `phase`, `animatedContainerStyle?`, `animatedLabelStyle?` | Business-free selected navigation surface: one absolutely positioned 48px capsule that uses the 6% black fill only when resting and remains plain glass while travelling or settling |
