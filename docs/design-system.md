...
| `BottomNavBar` | _(none)_ | Figma-state app navigation: 240×60 for Home/Chats and 220×60 for Me; touch-down immediately starts one persistent selected icon/capsule, shell width, and state-specific inactive icon positions on one coordinated React Native Animated spring while the glass shell peaks uniformly at 1.12 over 90ms and returns over the next 220ms, overlapping capsule travel; the capsule does not add a second scale; origin/destination stable icons stay hidden during travel; the 160ms label reveal scales from its icon-facing edge; Me preserves the Navii avatar; native iOS glass with material-blur fallback |
...
| `PersistentNavigationCapsule` | `label`, `leadingVisual`, `frame`, `phase`, `animatedContainerStyle?`, `animatedContentStyle?`, `animatedFillStyle?`, `animatedLabelStyle?`, `outgoingLabel?`, `animatedOutgoingLabelStyle?` | Business-free selected navigation surface: one clipped, absolutely positioned 48px capsule whose `phase` automatically renders the exact 6% fill without travel decoration at rest and a neutral hairline/shadow clear glass while travelling/settling; content and fill may be animated independently, outgoing-label opacity is owned outside destination content, and native glass views are never nested |
