...
| `BottomNavBar` | _(none)_ | Figma-state app navigation: 240×60 for Home/Chats and 220×60 for Me; touch-down immediately starts one persistent selected icon/capsule and its centered shell width on one coordinated 280ms React Native Animated spring while the glass shell scales uniformly to 1.12 over 90ms; the capsule does not add a second scale; handoff overlaps labels only; Me preserves the Navii avatar; native iOS glass with material-blur fallback |
