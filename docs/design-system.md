...
| `PersistentNavigationCapsule` | `label`, `leadingVisual`, `frame`, `phase`, `animatedContainerStyle?`, `animatedContentStyle?`, `animatedFillStyle?`, `animatedLabelStyle?`, `outgoingLabel?`, `animatedOutgoingLabelStyle?` | Business-free selected navigation surface: one clipped, absolutely positioned 48px capsule with a neutral hairline/shadow clear-glass base that stays visible on white, independently animated content and 6% black overlay, and optional outgoing-label-only handoff; it does not nest native glass views |
