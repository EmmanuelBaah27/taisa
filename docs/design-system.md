...
| `BottomNavBar` | _(none)_ | Figma-state app navigation: 240×60 for Home/Chats and 220×60 for Me; all three tab identities remain mounted above a content-free moving glass capsule; each tab frame expands/collapses on the coordinated spring while its full label fades and scales outward from the icon-facing edge over 160ms (never width-clipped); navigation mounting waits 220ms so route rendering cannot interrupt the opening motion; no tab icon is hidden or exchanged; the shell peaks uniformly at 1.12 over 90ms and returns over the next 220ms during capsule travel; Me preserves the Navii avatar; native iOS glass with material-blur fallback |
