...
| `BottomNavBar` | _(none)_ | Figma-state app navigation: 240×60 for Home/Chats and 220×60 for Me; one persistent selected capsule moves on a coordinated 280ms spring while the entire glass shell scales uniformly to 1.12 in 90ms; Me preserves the Navii avatar; native iOS glass with material-blur fallback |
...
| `PersistentNavigationCapsule` | `label`, `leadingVisual`, `frame`, `phase`, `animatedContainerStyle?`, `animatedLabelStyle?` | Business-free selected navigation surface: one absolutely positioned 48px capsule that uses the 6% black fill only when resting and a runtime-safe clear sheen/highlight border while travelling or settling; it does not nest native glass views |
